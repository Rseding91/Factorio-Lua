...
static GCObject **sweeplist (lua_State *L, GCObject** list, GCObject **p, lu_mem count);

static void sweepwholelist(lua_State* L, GCObject** list)
{
  GCObject *p = *list;
  sweeplist(L, list, &p, MAX_LUMEM);
}
...
*/
static GCObject **sweeptolive (lua_State *L, GCObject** list, GCObject **p, int *n) {
  GCObject ** old = p;
  int i = 0;
  do {
    i++;
    p = sweeplist(L, list, p, 1);
  } while (p == old);
  if (n) *n += i;
  return p;
}
...
static GCObject *udata2finalize (global_State *g) {
  ...
  lua_unlinkgcobject(&g->tobefnz, o);
  /* return it to 'allgc' list */
  lua_linkgcobject(&g->allgc, o);
  ...
  while ((curr = *p) != NULL) {  /* traverse all finalizable objects */
    ...
    if (!(all || iswhite(curr)))  /* not being collected? */
      ...
      *p = gch(curr)->next;
      ...
      lua_unlinkgcobject(&g->finobj, curr);
...
void luaC_checkfinalizer (lua_State *L, GCObject *o, Table *mt) {
  ...
  if (testbit(gch(o)->marked, SEPARATED) || /* obj. is already separated... */
    ...
    if (g->sweepgc == &ho->next) {  /* avoid removing current sweep object */
      lua_assert(issweepphase(g));
      g->sweepgc = sweeptolive(L, &g->allgc, g->sweepgc, NULL);
    }

    /* unlink object from allgc list */
    lua_unlinkgcobject(&g->allgc, o);

    /* link it in list 'finobj' */
    lua_linkgcobject(&g->finobj, o);
...
static int entersweep (lua_State *L) {
  ...
  {
    GCObject *p = g->finobj;
    g->sweepfin = sweeptolive(L, &g->finobj, &p, &n);
  }
  {
    GCObject *p = g->allgc;
    g->sweepgc = sweeptolive(L, &g->allgc, &p, &n);
  }
  return n;
}
...
static l_mem atomic (lua_State *L) {
  global_State *g = G(L);
  l_mem work = -(ptrdiff_t)g->GCmemtrav;  /* start counting work */
...
static lu_mem singlestep (lua_State *L) {
  ...
  switch (g->gcstate) {
    ...
    case GCSsweepudata: {
      if (g->sweepfin) {
        g->sweepfin = sweeplist(L, &g->finobj, g->sweepfin, GCSWEEPMAX);
    ...
    case GCSsweep: {
      if (g->sweepgc) {
        g->sweepgc = sweeplist(L, &g->allgc, g->sweepgc, GCSWEEPMAX);
        return GCSWEEPMAX*GCSWEEPCOST;
      }
      else {
        /* sweep main thread */
        GCObject *mt1 = obj2gco(g->mainthread);
        GCObject *mt2 = obj2gco(g->mainthread);
        sweeplist(L, &mt1, &mt2, 1);
        checkSizes(L);
