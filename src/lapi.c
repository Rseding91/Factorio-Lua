...
LUA_API int lua_isvalidIndex (lua_State* L, int idx)
{
  ...
  if (LUA_REGISTRYINDEX - idx <= MAXUPVAL + 1)
    return 1;
...
// Try to grow the stack to hold at least @size elements
static int checkstack_locked(lua_State* L, int size)
{
  ...
  if (res && callInfo->top < L->top + size)
    callInfo->top = L->top + size; // adjust frame top
  return res;
}
...
{
  lua_lock(L);
  int res = checkstack_locked(L, size);
  lua_unlock(L);
  return res;
}
...
LUA_API const char *lua_tolstring (lua_State *L, int idx, size_t *len) {
  ...
  if (!ttisstring(o)) {
    ...
    checkstack_locked(L, 1);
    o = index2addr(L, idx); // checkstack may re-allocate the stack
...
LUA_API void lua_rawsetglobal (lua_State *L, const char *var) {
  ...
  setobj2t(L, luaH_set(L, hvalue(gt), L->top-1), L->top-2);
  invalidateTMcache(hvalue(gt));
  luaC_barrierback(L, gcvalue(gt), L->top-2);
...
LUA_API int lua_load (lua_State *L, lua_Reader reader, void *data,
  ...
  if (status == LUA_OK) {  /* no errors? */
    ...
    if (f->nupvalues == 1) {  /* does it have one upvalue? */
      ...
      Table *reg = hvalue(&G(L)->l_registry);
      const TValue *gt = luaH_getint(L, reg, LUA_RIDX_GLOBALS);
...
void lua_tableresize(lua_State* L, int idx, int narr, int nrec)
{
  lua_lock(L);
  StkId t = index2addr(L, idx);
  api_check(L, ttistable(t), "table expected");
  luaH_resize(L, hvalue(t), narr, nrec);
  lua_unlock(L);
}
